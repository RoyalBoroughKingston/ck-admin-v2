<template>
  <gov-width-container>
    ...
    <template v-else>
      <vue-headful
        :title="
          `One Hounslow Connect - Edit User: ${user.first_name} ${user.last_name}`
        "
      />
      ...
      <gov-main-wrapper>
        <gov-grid-row>
          <gov-grid-column width="one-half">
            ...
            <gov-body
              >Edit users who can acces the back-end of the One Hounslow Connect
    ...
    methods: {
      async fetchUser() {
        ...
        if (this.form.roles.find((role) => role.role === 'Super Admin')) {
          ...
          this.form.roles.find((role) => role.role === 'Global Admin')
        ) {
